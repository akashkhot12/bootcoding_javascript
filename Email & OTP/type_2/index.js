...
function trueFalse(){
  ...
  btnOTP.addEventListener('click', () => {
    if (btnIpt.value == OTP) {
      ...
      result.innerHTML = `<label for="exampleFormControlInput1" class="fs-1">*Email verified</label>`
      result.style.color = "green"
    }
    else {
      let result = document.getElementById("result")
      result.innerHTML = `<label for="exampleFormControlInput1 class="fs-1">*Invalid Input</label>`
...
function sendEmail(e) {
  ...
  Email.send({
    ...
    message => {
      if (message === "OK") {
        result.innerHTML = `<label for="exampleFormControlInput1" class="fs-1">OTP send</label>`
